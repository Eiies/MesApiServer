...
public class DeviceService(IDeviceRepository repository, IMesAdapter mesAdapter, ILogger<DeviceService> logger) :IDeviceService {
    ...
    public void HandleTrackIn(TrackInRequest request) {
        // 发送消息到MES
        mesAdapter.SendMessage($"Device {request.DeviceId} tracked in product {request.ProductCode} at {request.ProductCode}");
        repository.SaveTrackIn(request);
    }

    public void HandleEQPConfirm(EQP2DConfirmRequest request) {
        // 发送消息到MES
        mesAdapter.SendMessage($"Device {request.DeviceId} confirmed barcode {request.Barcode} at {request.Barcode}");
        repository.SaveEQPConfirm(request);
    }

    public void HandleProcessEnd(ProcessEndRequest request) {
        // 发送消息到MES
        mesAdapter.SendMessage($"Device {request.DeviceId} process ended with result {request.Result} at {request.Result}");
