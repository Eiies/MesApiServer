...
namespace ApiServer.Repositories {
    public class DeviceRepository(AppDbContext context, ILogger<DeviceRepository> logger) :IDeviceRepository {
        public void SaveHeartbeat(Device device) {
            Device? existingDevice = context.Devices.FirstOrDefault(d => d.DeviceId == device.DeviceId);
        ...
        public void SaveTrackIn(TrackInRequest r) {
            ...
            Device device = EnsureDeviceExists(r.Content.EQPID);
            TrackIn trackIn = new() {
        ...
        private Device EnsureDeviceExists(string deviceId) {
            Device? device = context.Devices.FirstOrDefault(d => d.DeviceId == deviceId);
            if(device == null) {
                device = new Device {
                    DeviceId = deviceId,
